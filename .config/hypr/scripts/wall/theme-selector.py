...
rofi_process = subprocess.Popen(["rofi","-width","1000px","-dmenu","-format","i","-i","-markup-rows","-mesg","Type theme name:","-window-title",""],
...
def parse_theme(theme:str,dir:str,message=""):
    ...
    return f"{output} {theme}"

def write_to_rofi(input:str):
    rofi_process.stdin.write(str.encode(input+'\n'))
